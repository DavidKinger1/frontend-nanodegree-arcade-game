...
var Enemy = function(spriteImg) {
    ...
    this.speedMulti = 1+(1.0 * Math.random()); // speed multipler to apply to different movement speed
    this.lane = Math.round(2*Math.random()+1);    // just want numbers 1-3
    this.row = this.lane;   // use the row as a better way to keep track available lanes
    this.col = 0;   // same with column
    this.active =true; // used to determine if enemy is in play
    // The image/sprite for our enemies, this uses
    // a helper we've provided to easily loads images
...
Enemy.prototype.update = function(dt) {
    ...
    if (this.col > 4) {
        //reset the row and apply a new random lane to enemy
        this.col=0;
        this.lane = Math.round(2*Math.random()+1);
    }
    //this.row += Math.round(this.row * dt);
